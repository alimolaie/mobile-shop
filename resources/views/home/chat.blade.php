        ...
        <div class="chatContents">
            <div class="chatContent" id="chatContent1">
                ...
                <div class="body content1">
                    ...
                    @if(\App\Models\Setting::where('key' , 'ticketStatus')->pluck('value')->first())
                        <div class="choice" data-id="chatContent2">پیام به پشتیبانی</div>
                    @endif
                    @if(\App\Models\Setting::where('key' , 'chatStatus')->pluck('value')->first())
                        <div class="choice" data-id="chatContent3">گفتگو آنلاین</div>
                    @endif
                    @foreach(\App\Models\FloatAccess::get() as $item)
                        <a href="{{$item->link}}" target="_blank" class="choice">
                            {{$item->title}}
                        </a>
                    @endforeach
                </div>
