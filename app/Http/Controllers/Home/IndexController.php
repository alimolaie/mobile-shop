...
class IndexController extends Controller
{
    ...
    public function index(Request $request){
        ...
        if($cacheStatus){
            $widget = Cache::remember('widgets_cache_key2', $cacheTime, function () use($request) {
                $Agent = new Agent();
                $widgetsR = Widget::where('status', 1)->where('language', request()->cookie('language') ?? 'fa')->where('responsive', 1)->count();
        ...
        $storySeen = empty($storySeen = Cookie::get('story')) ? [] : explode(',', $storySeen);
    ...
    public function lucky(){
        if (auth()->user()){
            ...
            $gifts = Gift::whereDate('created_at','>=',Carbon::today()->addDay(-$maxGift))->where('type',1)->whereDate('created_at','<=',Carbon::today())->where('user_id' , Auth::id())->first();
            if($gifts){
    ...
    public function getProduct(Request $request){
        ...
        if($category){
            return $category->product()
                ...
                ->where('language', request()->cookie('language') ?? 'fa')
                ->when($request->option == 0, fn ($query) => $query->latest())
                ->when($request->option == 2, fn ($query) => $query->withCount('payMeta')->orderBy('pay_meta_count', 'DESC'))
                ->when(in_array($request->option, [1, 3]), fn ($query) => $query->withCount('view')->orderByDesc('view_count'))
                ->when(in_array($request->option, [4, 5]), fn ($query) => $query->orderBy('price', $request->option == 4 ? 'asc' : 'desc'))
                ->where('status', 1)
                ->where('variety', 0)
                ->take(10)
                ->get(['id','price','image','count','slug','imageAlt','product_id','title','titleEn']);
        }
        return [];
    }
    ...
    public function ticket(){
        $title = \request()->title;
        $tickets = \auth()->user() ? Ticket::where(function ($query){
            $query->where('customer_id',\auth()->user()->id)
                ->where('user_id',\auth()->user()->id);
        })->where('parent_id' , 0)
        ->when($title, function ($query) use($title) {
            return $query->where('id', $title);
        })->latest()->where('status' , 0)->get() : [];
        $category = Category::where('type',3)->get(['name','id']);
        $pays = \auth()->user() ? Pay::where('user_id',\auth()->id())->get(['property','created_at']):[];
        return view('home.faq.ticket',compact('tickets','category','pays','title'));
    }
    ...
    public function getWidget($widgets , $request){
        ...
        foreach ($widgets as $item){
            ...
            if($item['name'] != 'تبلیغ اسلایدری' && $item['name'] != 'محصولات افقی با دسته بندی' && $item['name'] != 'محصولات با فیلتر' && $item['name'] != 'درباره ما' && $item['name'] != 'بنر با عنوان' && $item['name'] != 'دسته بندی 2' && $item['name'] != 'مقایسه' && $item['name'] != 'جستجو' && $item['name'] != 'متن' && $item['name'] != 'سوال متداول' && $item['name'] != 'جستجو2' && $item['name'] != 'جشنواره' && $item['name'] != 'گردونه دسته بندی' && $item['name'] != 'اسلایدر بزرگ' && $item['name'] != 'بهترین ها' && $item['name'] != 'تبلیغ ساده' && $item['name'] != 'فروشندگان' && $item['name'] != 'خبر' && $item['name'] != 'خبر2'  && $item['name'] != 'دسته بندی'){
                $ids = [];
                $ids2 = [];
                if($item['cats'] != '[]'){
                ...
                switch ($item['sort']) {
                    case 0:
                        $catPost1 = $commonQuery->latest()
                            ...
                            ->get(['id','off','price','image','type','count','slug','minCart','maxCart','offPrice','imageAlt','colors','product_id','lotteryStatus','title','inquiry','ability','short']);
                        break;
                    ...
                    case 3:
                        $catPost1 = $commonQuery->orderBy('price')->get(['id','off','price','image','type','count','slug','minCart','maxCart','product_id','offPrice','imageAlt','colors','lotteryStatus','title','inquiry','ability','short']);
                        break;

                    case 4:
                        $catPost1 = $commonQuery->orderBy('price', 'DESC')->get(['id','off','price','image','type','count','slug','minCart','product_id','maxCart','offPrice','imageAlt','colors','lotteryStatus','title','inquiry','ability','short']);
                        break;
                    ...
                    case 5:
                        $catPost1 = $commonQuery
                            ...
                            ->get(['id','off','price','image','type','count','slug','minCart','maxCart','offPrice','imageAlt','colors','lotteryStatus','product_id','title','inquiry','ability','short']);
                        break;
                }
            ...
            if($item['name'] == 'دسته بندی'){
                ...
                $allCatSite3 = json_decode($item['cats'], true);
                foreach ($allCatSite3 as $value){
                    ...
                    if($tax){
                        ...
                        $categoryPost['product'] = $tax->product()->where('language' , request()->cookie('language')??'fa')->select(['slug','image','product_id','titleSeo','title'])->take(4)->get();
                        array_push($widgetCategory['post'],$categoryPost);
                    }
                }
            }
            if($item['name'] == 'دسته بندی 2'){
                $allCatSite3 = json_decode($item['cats'], true);
                $widgetCategory['post'] = Category::whereIn('id' , $allCatSite3)->where('language' , request()->cookie('language')??'fa')->get(['name','image','slug']);
            }
            if($item['name'] == 'محصولات افقی با دسته بندی' || $item['name'] == 'محصولات با فیلتر'){
                $allCatSite3 = json_decode($item['cats'], true);
                $catD = Category::whereIn('id' , $allCatSite3)->where('language' , request()->cookie('language')??'fa')->first();
                if($catD){
                    $widgetCategory['post'] = $catD->product()->orderByRaw('count > 0 DESC')->latest()->where('language', request()->cookie('language') ?? 'fa')->where('status', 1)->where('variety', 0)->take(10)->get(['id','price','image','count','slug','imageAlt','product_id','title','titleEn']);
                }
            }
            ...
            if($item['name'] == 'خبر' || $item['name'] == 'خبر2'){
                $widgetCategory['post'] = News::latest()->take(8)->where('status',1)->where('language' , request()->cookie('language')??'fa')->select(['title' , 'slug' , 'imageAlt' , 'bodySeo' , 'image'])->get();
            }
            if($item['name'] == 'استوری'){
                $widgetCategory['post'] = Story::orderBy('id', 'desc')->with('children')->where('parent_id',0)->get();
            }
    ...
    public function sendCounseling(Request $request){
        ...
        if($messageCounseling){
            $this->sendSms($number , [$request->number , $product->title],env('GHASEDAKAPI_Number'),$messageCounseling);
        }
        Counseling::create([
            'body'=> $request->body,
            'user_id'=> $user,
