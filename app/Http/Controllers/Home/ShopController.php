...
{
    use SendSmsTrait;
    public function getCarrier($car,$amount,$weight1){
        $weights = $weight1/1000;
        if($weight1 == 0){
            if(\auth()->user()){
                foreach (Cart::where('user_id' , auth()->user()->id)->where('number' , 0)->get(['product_id','count']) as $val){
                    $weights += (Product::where('id',$val->product_id)->value('weight') * $val->count)/1000;
                }
            }else{
                ...
                if(!empty($myCart)){
                    if ($myCart){
                        foreach(json_decode($myCart , true) as $item) {
                            $weights += (Product::where('id',$item['id'])->value('weight') * $item['count'])/1000;
                        }
                    }
                }
            }
        ...
        if($sends1){
            if($sends1['limit'] <= $amount + (int)($weights*$sends1['weightPrice'])){
                return 0;
            }else{
                if(\auth()->user()){
                    if(CarrierCity::where('carrier' , $car)->where('city' , auth()->user()->address()->where('show' , 1)->where('status',1)->pluck('city')->first())->first()){
                        return CarrierCity::where('carrier' , $car)->where('city' , auth()->user()->address()->where('show' , 1)->where('status',1)->pluck('city')->first())->pluck('price')->first() + (int)($weights*$sends1['weightPrice']);
                    }else{
                        return $sends1['price'] + (int)($weights*$sends1['weightPrice']);
                    }
                }else{
                    return $sends1['price'] + (int)($weights*$sends1['weightPrice']);
                }
    ...
    public function installments(Request $request){
        ...
        if(!$request->time && Setting::where('key' , 'deliverStatus')->pluck('value')->first() == 1){
            return redirect()->back()->with([
        ...
        if (auth()->user()->cart()->where('number' , 0)->count() >= 1) {
            ...
            $sends = $this->getCarrier($request->carrier,$amount,0);
    ...
    public function card(Request $request){
        $checkPhysical = $this->checkPhysical();
    ...
    public function quickBuy(Request $request)
    {
        ...
        if($product['colors']){
            foreach(json_decode($product['colors']) as $item){
                if($item->name == $myData->color){
                    ...
                    if($myData->count > $item->count){
                        return 'رنگ ناموجود است';
                    }
                }
            }
        }
        ...
        $sends = $this->getCarrier($myData->carrier,$amount,$product->weight);
        $amount = (int)$amount + (int)$sends;
        ...
        if (auth()->user()->cart()->where('number' , 0)->count() >= 1) {
            ...
            if($count[0]->discount){
                ...
                if($discount){
                    ...
                    $disAmount = ($amount * $discount->percent) / 100;
                    $amount = $amount - ($amount * $discount->percent) / 100;
                }
            }
            $cartList = $checkCart[2];
            $cartList['totalAmount'] = ((int)$amount + (int)$disAmount) *10;
            $cartList['shippingAmount'] = $sends*10;
            $numbers = \auth()->user()->number?? '09111111111';
            $numbers = preg_replace('/0/', '+98', $numbers, 1);
            $ss = [
                'amount'=> $amount*10,
                'cartList' => [$cartList],
                'discountAmount' => $disAmount*10,
                'externalSourceAmount'  => 0,
                'mobile' => $numbers,
                'paymentMethodTypeDto' => 'INSTALLMENT',
                'returnURL' => url('/order/snappay'),
                'transactionId' => time()
            ];
            ...
            if($choicePay == 9){
                ...
                $client_id = $idSnap;
                $client_secret = $secretSnap;
                $token = 'Basic ' . base64_encode($client_id . ':' . $client_secret);

                $client = new Client();
                $response = $client->request('POST', 'https://api.snapppay.ir/api/online/v1/oauth/token',
                    [
                        'headers' => array(
                            'Authorization' => $token,
                ...
                curl_setopt_array($curl, array(
                    ...
                    CURLOPT_POSTFIELDS =>json_encode($ss),
                ...
                curl_close($curl);
                $pay->update([
                    'auth' => json_decode($response,true)['response']['paymentToken'],
                ]);
                session()->put('transactionId' , json_decode($response,true)['response']['paymentToken']);
                session()->put('amount' , $amount);
                session()->put('user_ids' , \auth()->user()->id);
                return redirect(json_decode($response,true)['response']['paymentPageUrl']);
            }
            $invoice = (new Invoice)->amount($amount);
            ...
            return Payment::via($gate)->config($configs)->callbackUrl(url('/order'))->purchase(
                $invoice,
                function($driver, $transactionId) use ($property,$checkPhysical,$amount,$tax,$choicePay,$sends,$carrierData,$request,$count) {
                    $time = Carbon::now()->format('Y-m-d h:i:s');
                    $address = auth()->user()->address()->where('show' , 1)->where('status' , 1)->first();
                    $pay = Pay::create([
                        'refId'=>'',
    ...
    public function snappay(Request $request)
    {
        $transaction_id = session()->get('transactionId');
        if(Auth::check()){
            $pay = Pay::where('auth' , $transaction_id)->where('user_id' , auth()->user()->id)->first();
        }else{
            $pay = Pay::where('status',0)->latest()->where('user_ip',$request->ip())->first();
            Auth::login(User::where('id' , $pay->user_id)->first());
        }
        ...
        if($request->state != 'OK'){
            ...
            return redirect('/checkout')->with([
                'message' => 'سفارشتان پرداخت و ثبت نهایی نشد'
            ]);
        }
        curl_setopt_array($curl, array(
        ...
        $response = curl_exec($curl);
        curl_close($curl);
        $refs = json_decode($response,true)['response']['transactionId'];
        if(json_decode($response,true)['successful'] != 'true'){
            $curl = curl_init();
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://api.snapppay.ir/api/online/payment/v1/revert',
                CURLOPT_RETURNTRANSFER => true,
        ...
        $pay->update([
            'status' => 100,
            'price' => $pay->price/10,
            'refId' => json_decode($response,true)['response']['transactionId'],
        ]);
        foreach ($pay->payMeta as $val){
            $val->update([
                'status' => 100,
    ...
    public function checkCart($count){
        ...
        for ($i = 0; $i < count($count); $i++) {
            if($count[$i]->pack == 1) {
                ...
                if($post){
                    $price = $post->price;
                    foreach (json_decode($post->options) as $val){
                        foreach (json_decode($count[$i]['options']) as $el){
                            if($val->title == $el){
                                $price += $val->price;
                            }
                        }
                ...
                array_push($widgetCategory['cartItems'],$ps);
            }
        ...
        return ['success' , 'ok',$widgetCategory];
    }
                    ...
                    for ( $i = 0; $i < $item->count; $i++) {
                        ...
                        if($lastLottery1 != ''){
                            if($post->numLottery2 != $lastLottery1->number){
                                ...
                                $message = "$registerMessage <br> $code->code";
                                SendEmailJob::dispatch($message , __('messages.lottery_code2') , $user->email);
                            }
                        }
                        else{
                            $code = LotteryCode::create([
    ...
    {
        $physical = 0;
        $cart = Cart::where('user_id' , auth()->user()->id)->where('number' , 0)->get();
        foreach ($cart as $item){
            if($item->collect_id){
                $physical = 1;
            }else{
                $send = Product::where('id', $item->product_id)->first();
                if($send->type == 0){
                    $physical = 1;
                }
            }
