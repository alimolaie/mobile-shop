...
class CartController extends Controller
{
    ...
    public function addCart(Request $request){
        ...
        $limit = 0;
        if (auth()->user()) {
            $check = Cart::where('product_id', $request->product)->where('pack',0)->where('size', $request->size)->where('color', $request->color)->where('guarantee_id', $request->guarantee)->where('user_id', auth()->user()->id)->first();
            if ($check) {
                if($product->type == 0){
                    if($product['colors']){
                        foreach(json_decode($product['colors']) as $item){
            ...
            if(!empty($myCart)){
                if ($myCart){
                    foreach(json_decode($myCart , true) as $item) {
                        if ($item['pack'] == 0 && $item['id'] == $request->product && $request->size == $item['size'] && $request->color == $item['color'] && $request->guarantee == $item['guarantee_id']) {
                            if($product->type == 0) {
                                ...
                                if ($product['colors']) {
                                    foreach (json_decode($product['colors']) as $val) {
                                        if ($val->name == $request->color) {
                                            $price = (int)$price + (int)$val->price;
                    ...
                    if ($product->type == 0){
                        ...
                        $filtered = $c->where('id', '=' , $request->product)->first();
                    }
                    if(!$filtered){
                        $cart=[
                            'id' => $request->product,
    ...
    public function addPack(Request $request){
        ...
        if (auth()->user()) {
            ...
            if ($check) {
                $check->update([
                    'count' => ++$check->count,
                    'price' => $request->price >= 1?$request->price:$product->price,
            ...
            for ( $i = 0; $i < count($changeCart); $i++) {
                if($changeCart[$i]['number'] == 0){
                    if($changeCart[$i]['pack']){
                        $send = Collection::where('id', $changeCart[$i])->with('user')->first();
                    }else{
                        $send = Product::where('id', $changeCart[$i])->with('user')->first();
                    }
                    $data = [
    ...
    public function addCartFast(Request $request)
    {
        ...
        if (auth()->user()) {
            ...
            if(!empty($myCart)){
                if ($myCart){
                    foreach(json_decode($myCart , true) as $item) {
                        if ($item['pack'] == 0 && $item['id'] == $request->product && $sizeN == $item['size'] && $colorN == $item['color'] && $guarantee == $item['guarantee_id']) {
                            if($post->type == 0){
    ...
    public function addCartFast2(Request $request)
    {
        ...
        if (auth()->user()) {
            ...
            if(!empty($myCart)){
                if ($myCart){
                    foreach(json_decode($myCart , true) as $item) {
                        if ($item['id'] == $request->product && $item['pack'] == 0 && $sizeN == $item['size'] && $colorN == $item['color'] && $guarantee == $item['guarantee_id']) {
                            if($post->type == 0){
                                if($post->count <= $item['count'] && $post->prebuy == 0){
                                    return 'limit';
                                }
    ...
    public function checkout(){
        ...
        $carriers = Carrier::where('weight','>=',$weights)->latest()->get();
        ...
        if($statusSnap){
            ...
            curl_setopt_array($curl, array(
                CURLOPT_URL => 'https://api.snapppay.ir/api/online/offer/v1/eligible?amount='.$prices*10,
            ...
            $eligible = json_decode($response,true);
        }
        return view('home.cart.checkoutIndex' , compact('carts','prebuy','physical','weights','eligible','cartPrice','statusSnap','statusSaman','taxPrice','tax','cardText','statusAsan','statusPasargad','zarinpalStatus','zibalStatus','nextpayStatus','idpayStatus','statusBeh','statusSadad','wallet','card','count','packs','spot','gateway','installment','carriers','days','address','scores','prices'));
    }
    ...
    public function setToCart(){
        ...
        for ( $i = 0; $i < count($carts2); $i++) {
            if($carts2[$i]['pack'] == 1){
                $send = Collection::where('id', $carts2[$i]['collect_id'])->where('count' , '>=' , $carts2[$i]['count'])->with('user')->first();
                if($send){
                    $price = $send->price;
                    foreach (json_decode($send->options) as $val){
                        foreach (json_decode($carts2[$i]['options']) as $el){
                            if($val->title == $el){
                                $price += $val->price;
                            }
                        }
            ...
            if(!$send){
                ...
                $data = [
                    ...
                    'inquiry' => $carts2[$i]['inquiry'],
                    'size' => $carts2[$i]['size'],
                    'color' => $carts2[$i]['color'],
                    'price' => $price,
                    'options' => $carts2[$i]['options'],
                    'guarantee_id' => $carts2[$i]['guarantee_id'],
                    'guarantee' => $carts2[$i]['guarantee_id'] >=1 ? Guarantee::where('id',$carts2[$i]['guarantee_id'])->first() : '',
                ];
                array_push($carts, $data);
            }
        };
    ...
    public function changeSnap(Request $request){
        $userSnap = Setting::where('key' , 'userSnap')->pluck('value')->first();
        $passwordSnap = Setting::where('key' , 'passwordSnap')->pluck('value')->first();
        $idSnap = Setting::where('key' , 'idSnap')->pluck('value')->first();
        $secretSnap = Setting::where('key' , 'secretSnap')->pluck('value')->first();
        ...
        $client = new Client();
        $response = $client->request('POST', 'https://api.snapppay.ir/online/v1/oauth/token',
            [
                'headers' => array(
                    'Authorization' => $token,
                    'Content-Type'  => 'application/x-www-form-urlencoded',
        ...
        curl_setopt_array($curl, array(
            CURLOPT_URL => 'https://api.snapppay.ir/online/offer/v1/eligible?amount='.$request->price*10,
            CURLOPT_RETURNTRANSFER => true,
            CURLOPT_ENCODING => '',
            CURLOPT_MAXREDIRS => 10,
            CURLOPT_TIMEOUT => 0,
        ...
        $response = curl_exec($curl);
        return $eligible = json_decode($response,true);
    }
}
