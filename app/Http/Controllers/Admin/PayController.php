...
class PayController extends Controller
{
    ...
    public function updatePaySnap(Pay $pay , Request $request){
        $checkCart = $this->checkCart($pay);
        $cartList = $checkCart[2];
        $cartList['totalAmount'] = ((int)$pay->price) *10;
        $cartList['shippingAmount'] = $pay->carrier_price*10;
        $numbers = \auth()->user()->number?? '09111111111';
        $numbers = preg_replace('/0/', '+98', $numbers, 1);

        $dis = Product::whereIn('id',$pay->payMeta()->where('cancel',0)->pluck('product_id')->toArray())->sum('offPrice') + (int)$pay->carrier_price;
        $ss = [
            'amount'=> $pay->price*10,
            'cartList' => [$cartList],
            'discountAmount' => ($dis - $pay->price)*10,
    ...
    public function checkCart($pay){
        $count = $pay->payMeta()->where('cancel',0)->get();
        $widgetCategory = [
            'cartId'=> time(),
    ...
    public function addPay(Pay $pay , Request $request){
        $pi2 = Product::where("id",$request->productM)->first();
        PayMeta::create([
            ...
            'profit' => (int)$pi2['price'] - $pi2->priceBuy,
            'user_id' => $pay->user_id,
            'status' => $pay->status,
            'pay_id' => $pay->id,
            'deliver' => $pay->deliver,
    ...
    public function group(Request $request){
        ...
        $payMetas=PayMeta::whereIn('pay_id',explode(',',$request->pay))->get();
        return view('admin.pay.prints',compact('pays','payMetas'));
    }
    ...
    public function createP(Request $request){
        ...
        foreach (json_decode($request->metas) as $item){
            $pi2 = Product::where('id',$item->product)->first();
    ...
    public function chart(Request $request){
        ...
        $tops = Product::withCount([
            "payMeta" => function($q) {
                $q->whereNotIn('status', [0, 1]);
            },
            "payMeta as payMeta2" => function($q) use ($startDayEn, $endDayEn) {
                $q->whereBetween('created_at', [$startDayEn, $endDayEn])->whereNotIn('status', [0, 1, 2]);
            }
        ])
            ->orderBy('payMeta2', 'DESC')
            ->take(3)
            ->get();

        $views = Product::withCount([
            'view',
            "view as view2" => function($q) use ($startDayEn, $endDayEn) {
                $q->whereBetween('created_at', [$startDayEn, $endDayEn]);
            }
        ])
            ->orderBy('view2', 'DESC')
            ->take(3)
