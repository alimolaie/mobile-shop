...
class PostController extends Controller
{
        ...
        if($request->title){
            $products = Product::where(function ($query) use($title) {
                $query->where('title' , "LIKE" , "%{$title}%")
                    ->orWhere('id', $title);
            })->select(['id' , 'title' ,'titleSeo','bodySeo','slug' ,'type','off','price' ,'product_id' , 'image' , 'slug' , 'count','created_at'])->where('variety' , 0)->latest()->paginate(50)->setPath($currentUrl);
        }else{
            $products = Product::select(['id' , 'product_id' , 'title' ,'type','off','titleSeo','bodySeo','slug' ,'price' , 'slug' , 'image' , 'count','created_at'])->where('variety' , 0)->latest()->paginate(50)->setPath($currentUrl);
        }
        return view('admin.post.index',compact('products','title'));
    }
    public function setting(Request $request){
        return view('admin.post.setting');
    }
    public function settingStore(Request $request){
        ...
        if($request->type == 4){
            ...
            foreach ($pp as $item){
                if ($request->off){
                    $price = round((int)$item->price - ((int)$item->price * $request->off / 100));
                }else{
    ...
    public function store(Request $request){
        ...
        $post = Product::create([
            ...
            'size' => $request->sizes,
            'varName1' => $request->varName1,
            'varName2' => $request->varName2,
            'rate' => $request->rates,
    ...
    public function seo(Product $product){
        ...
        $internalLinks = [];
        $externalLinks = [];
        $posts = Product::where('id' , $product->id)->first();
        if(strlen($posts->body) >= 30){
            $dom = new Dom;
            $dom = $dom->loadStr($posts->body);
            foreach ($dom->find('a') as $item){
                $check = explode($url , $item->getAttribute('href'));
                if(count($check) >= 2){
                    $link = [
                        'tag' => (string)$item,
        ...
        return view('admin.post.seo',compact('posts','internalLinks','externalLinks'));
    }
    public function seoLink(Product $product , Request $request){
        if(strlen($product->body) >= 30){
            $url = str_replace(['http://', 'https://'], '', url(''));
            $dom = new Dom;
            $dom = $dom->loadStr($product->body);
            for ( $i = 0 ; $i < count($dom->find('a')); $i++){
                $dom->find('a',$i)->setAttribute('rel' , $request->rel[$i]);
            }
            $product->update([
                'body' => $dom
            ]);
        ...
        return redirect()->back()->with([
            'message' => 'لینک ها با موفقیت ویرایش شدند'
        ]);
    }
    public function getKeyword(Request $request){
        if($request->body){
            $body = strip_tags($request->body);
            $rake = RakePlus::create($body, 'fa_IR');
            ...
            return str_replace(['&zwnj','   ','  '],'',implode(',' , $data));
        }
        return '';
    }
