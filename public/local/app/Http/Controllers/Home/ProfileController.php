...
class ProfileController extends Controller
{
    ...
    public function subcategory(){
        ...
        $users = User::where('parent_id' , auth()->user()->referral)->where('parent_id','!=',0)->withCount(["cooperation" => function ($q) {
            $q->where('status' , 100)->where('type' , 0)->select(DB::raw('sum(price)'));
        }])->get();
        $referralUser = User::where('referral' , auth()->user()->parent_id)->where('parent_id','!=',0)->get();
    ...
    public function getCollect(){
        ...
        if(!Gift::where('user_id',auth()->id())->whereDate('created_at',now()->today())->where('type',3)->first()){
            Gift::create([
                'user_id' => auth()->id(),
                'type' => 3,
                'discount' => $scoreDay,
