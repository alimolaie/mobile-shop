...
class ArchiveController extends Controller
{
    ...
    public function mallChange(Request $request , User $user){
        ...
        if($checks == 0){
            if ($request->show == 0){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->latest()->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
            ...
            if ($request->show == 2){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->withCount('payMeta')->orderBy('pay_meta_count','DESC' )->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 1 or $request->show == 3){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->withCount('view')->orderBy('view_count','DESC' )->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 4){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->orderBy('price')->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 5){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->orderBy('price','DESC')->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 0){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->latest()->whereIn('id' , $arrayFilter)->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 2){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->withCount('payMeta')->orderBy('pay_meta_count','DESC' )->whereIn('id' , $arrayFilter)->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 1 or $request->show == 3){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->withCount('view')->orderBy('view_count','DESC' )->whereIn('id' , $arrayFilter)->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 4){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->orderBy('price')->whereIn('id' , $arrayFilter)->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
            ...
            if ($request->show == 5){
                $catPost = $user->product()->orderByRaw('count > 0 DESC')->where('language' , request()->cookie('language')??'fa')->orderBy('price','DESC')->whereIn('id' , $arrayFilter)->where('status' , 1)->where('variety' , 0)->withCount('view')->withCount('comments')->withCount(["rates" => function ($q) {
                    $q->select(DB::raw('round(avg(rate),1)'));
                }])->withCount(["payMeta" => function ($q) {
                    $q->whereIn('status' , [100,20,50])->select(DB::raw('sum(count)'));
                }])->paginate(36)->setPath($currentUrl);
